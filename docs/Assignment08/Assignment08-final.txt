...
class IO:
    ...
    @staticmethod
    def GetsNewProductandPrice():
        global __product_name
        global __product_price
        # Step 3.2.a - Ask user for new product and price
        # ToDo/done: Place IO code in a new function
        __product_name = str(input("What is the product name? - ")).strip()  # Get task from user
        __product_price = str(input("What is the product price?- ")).strip()  # Get priority from user
        print()  # Add an extra line for looks

    @staticmethod
    def AddNewProductandPrice():
        # Step 3.2.b  Add item to the List/Table
        # ToDo/done: Place processing code in a new function
        dicRow = {"Product": __product_name, "Price": __product_price}  # Create a new dictionary row
...
while(True):
    ...
    # Step 3.2 - Add a new item to the list/Table
        ...
        IO.GetsNewProductandPrice() #Asks user for new task and priority

        # ToDo/done: Place processing code in a new function
        IO.AddNewProductandPrice() #Add New Task and Priority to list
        continue  # to show the menu
